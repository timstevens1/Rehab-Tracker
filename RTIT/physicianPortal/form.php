...
// form. Note it is best to follow the same order as declared in section 1c.
    ...
    $resultsInfo[] = $phone;
    $sDate = htmlentities($_POST['fldStartDate'], ENT_QUOTES, 'UTF-8');
    $resultsInfo[] = $sDate;
    $resultsInfo[] = $sDate;
...
//email checking
    ...
    if ($phone == "") {
        ...
        if (!preg_match($pattern,$sDate,$matches)) {
        $errorMsg[] = "Please enter a date (YYYY-MM-DD)";
        $phoneERROR = true;
    } elseif (!checkdate($matches[2], $matches[3], $matches[1])) {
        print_r($matches);
        $errorMsg[] = "Start date appears to be invalid.";
        $dateERROR = true;
    }
...
//
        ...
        <form action="<?php print $phpSelf; ?>"
            ...
            <!--<fieldset>-->
                ...
                <div class="form-group">
                    <label for="fldStartDate" class="required"><b> Patient Start Date</b>
                    <input type="text" id="fldStartDate" name="fldStartDate"
                           value="<?php print $sDate; ?>"
                           tabindex="130" maxlength="45" placeholder="YYYY-MM-DD"
                           <?php if ($phoneERROR) print 'class="mistake"'; ?>
                           onfocus="this.select()" 
                           autofocus>
                </label><br>
